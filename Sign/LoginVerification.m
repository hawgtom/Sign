...
@interface LoginVerification ()<MBProgressHUDDelegate>
{
    MBProgressHUD *HUD;
...
@implementation LoginVerification
@synthesize email;
@synthesize getemail;
- (void)viewDidLoad {
    [super viewDidLoad];
    ...
    self.mob.inputAccessoryView = numberToolbar;
    // Do any additional setup after loading the view.
}

-(void)cancelNumberPad{
    [self.mob resignFirstResponder];
}

-(void)doneWithNumberPad{
    NSString *numberFromTheKeyboard = self.mob.text;
    [self.mob resignFirstResponder];
}
                       ...
                       dispatch_async(dispatch_get_main_queue(), ^
                                      {
                                          ...
                                          @try{
                                              ...
                                              NSString *post =[[NSString alloc] initWithFormat:@"email=%@&mob=%@&status=%@",[self.email text],[self.mob text],@"1"];
                                              NSLog(@"PostData: %@",post);
                                              ...
                                              if ([response statusCode] >= 200 && [response statusCode] < 300)
                                              {
                                                  ...
                                                  if(success == 1)
                                                  {
                                                      [self alert:@"Verification Code Sent" :@"Signature" :1];
...
-(void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    if ([[segue identifier] isEqualToString:@"login_verification"]) {
        
        // Get destination view
         verification *vc = [segue destinationViewController];
        
        // Get button tag number (or do whatever you need to do here, based on your object
        vc.mob=[self.mob text];
    }
}
...
- (void) alert:(NSString *)msg :(NSString *)title :(int) tag
{
    ...
    UIAlertAction* ok = [UIAlertAction actionWithTitle:@"OK" style:UIAlertActionStyleDefault
                         ...
                         {
                             [self performSegueWithIdentifier:@"login_verification" sender:self];
                         }];
...
- (IBAction)sendcode:(id)sender {
    if([[self.mob text] isEqualToString:@""])
    {
